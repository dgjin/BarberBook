...
import { SystemSettings, Appointment, AppointmentStatus, Barber } from '../types';
import { DEFAULT_SETTINGS } from '../constants';
import { Save, Settings as SettingsIcon, Trash2, Loader2, RefreshCw, Database, Link, Users, Plus, Edit2, X, QrCode, Upload, Image as ImageIcon } from 'lucide-react';
...
export const AdminPanel: React.FC<AdminPanelProps> = () => {
  ...
  const cancelAppointment = async (id: string) => {
    if (confirm('确定要取消这个预约吗？')) {
      ...
      const updatedApps = await StorageService.getAppointments();
    ...
    <div className="max-w-4xl mx-auto p-4 space-y-8">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
        <div>
          ...
          <p className="text-xs text-gray-500">开放访问模式</p>
        </div>
      </div>
      ...
      <section className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex items-center gap-2 mb-6 border-b pb-4">
          ...
          <h2 className="text-xl font-bold text-gray-800">数据库连接 (Supabase)</h2>
        </div>
